export interface Project {
  id: number;
  title: string;
  description: string;
  longDescription: string;
  image: string;
  category: string;
  technologies: string[];
  status: "Completed" | "In Progress" | "Maintenance";
  year: string;
  features: string[];
  role: string;
}

export const projects: Project[] = [
  {
    id: 1,
    title: "Portofolio V3 ✨",
    description:
      "Personal portfolio website showcasing my profile, skills, experience, and projects as a Frontend Developer, enhanced with a real-time community chat room for interaction and feedback.",
    longDescription:
      "My personal portfolio website that represents my journey as a Frontend Developer, highlighting my skills, experiences, and featured projects. This website is designed with a strong focus on scalability, performance, and clean architecture to deliver an engaging and user-friendly experience. Built using modern web technologies such as React, Next.js, and TypeScript, the portfolio emphasizes responsive design, smooth animations, and intuitive navigation.\n\nIn addition to being a professional showcase, this portfolio also features a public community chat room where visitors can sign in using Google or GitHub to share feedback, ideas, or suggestions in real time. The chat system is powered by Supabase and NextAuth, demonstrating my ability to integrate authentication, database management, and real-time interactions into modern web applications. Overall, this project reflects my expertise in building performance-driven, interactive, and scalable web experiences that solve real-world problems ✨.",
    image: "/projects-1.webp",
    category: "Frontend Developer",
    technologies: [
      "Next.js",
      "React",
      "TypeScript",
      "Tailwind CSS",
      "Framer Motion",
      "GSAP",
      "Anime.js",
      "Three.js",
      "OGL",
      "Postprocessing",
      "Supabase",
      "NextAuth.js",
      "Lucide React",
      "Class Variance Authority (CVA)",
      "Tailwind Merge",
      "Responsive Web Design",
      "Modern Animation & Motion Effects",
      "Component-Based Architecture",
      "Authentication & Authorization",
      "Real-time Database Integration",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2026",
    features: [
      "Modern and responsive portfolio layout",
      "Advanced UI animations using Framer Motion, GSAP, and Anime.js",
      "Interactive 3D and visual effects with Three.js and OGL",
      "Smooth page transitions and motion-driven interactions",
      "Dark-themed interface with gradient accents",
      "Personal profile and professional summary section",
      "Experience and career timeline",
      "Project showcase with detailed information",
      "Technology stack and skills presentation",
      "Public community chat room",
      "Authentication with Google and GitHub",
      "Real-time message storage using Supabase",
      "Secure data handling with Row Level Security (RLS)",
      "Optimized performance with Next.js",
      "Clean, scalable, and maintainable code structure",
    ],
    role: "Frontend Developer",
  },
  {
    id: 2,
    title: "E-TICKET – Event Ticketing Platform 🎫🎉",
    description:
      "E-TICKET is a modern event ticketing platform that enables seamless event discovery, secure online ticket purchases, and digital QR code ticket management with real-time analytics.",
    longDescription:
      "**E-TICKET** is a modern digital event ticketing platform designed to streamline event discovery, ticket purchasing, and attendance management in one seamless experience. Users can easily browse and search for events, complete fast and secure ticket bookings with integrated Midtrans payments, and receive QR code–based e-tickets for reliable digital entry. For event organizers, the platform offers a real-time dashboard to track ticket sales and attendance efficiently, helping ensure smooth event operations. Built with scalability, security, and performance in mind, E-TICKET delivers a reliable solution for managing events of any size. 🚀",
    image: "/projects-2.webp",
    category: "Full Stack Developer",
    technologies: [
      "Next.js",
      "React",
      "TypeScript",
      "Prisma ORM",
      "PostgreSQL",
      "Tailwind CSS",
      "RESTful API",
      "JWT Authentication",
      "Midtrans Payment Gateway",
      "QR Code Generation",
      "Server Actions",
      "Zod Validation",
      "Axios / Fetch API",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "User authentication & authorization",
      "Event discovery with smart search and filters",
      "Online ticket booking & checkout",
      "Secure payment integration with Midtrans",
      "QR code–based e-ticket generation",
      "Digital ticket delivery",
      "Real-time sales & attendance dashboard",
      "Event management for organizers",
      "Responsive design for all devices",
    ],
    role: "Full Stack Developer",
  },
  {
    id: 3,
    title: "Food Ordering Management System 🍽️",
    description:
      "A web-based system that helps restaurants manage orders, menu items, and sales performance efficiently through a centralized dashboard.",
    longDescription:
      "The Food Ordering Management System is a web-based solution designed to streamline restaurant operations by centralizing order management, menu administration, and sales monitoring. The system allows staff to create and manage customer orders, track order status in real time, and maintain menu data with ease. Through an intuitive dashboard and detailed reports, restaurant owners can analyze sales performance and make informed, data-driven decisions. Built to improve efficiency and reduce operational friction, this system supports faster workflows and better control over daily restaurant activities.",
    image: "/projects-3.webp",
    category: "Full Stack Developer",
    technologies: [
      "Next.js",
      "React",
      "TypeScript",
      "Prisma ORM",
      "PostgreSQL",
      "Tailwind CSS",
      "RESTful API",
      "JWT Authentication",
      "Role-Based Access Control (RBAC)",
      "Server Actions",
      "Zod Validation",
      "Axios / Fetch API",
      "Dashboard & Data Visualization",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "User authentication & role-based authorization",
      "Order creation and management",
      "Real-time order status tracking",
      "Menu management (CRUD products & categories)",
      "Sales reports and analytics dashboard",
      "Daily and monthly sales summaries",
      "Staff workflow management",
      "Responsive dashboard for desktop and tablet",
    ],
    role: "Full Stack Developer",
  },
  {
    id: 4,
    title: "School Management System 🎓",
    description:
      "School Management System is a web-based platform that helps schools manage academic operations, student data, attendance, and payments efficiently through a centralized, real-time dashboard.",
    longDescription:
      "The School Management System is a web-based solution designed to streamline and centralize school administrative operations in real time. The system enables schools to monitor student distribution per class, track weekly attendance recaps, and analyze monthly payment statistics with ease. It also provides complete management of students, teachers, and class data through an intuitive interface. With centralized academic reports and real-time insights, this system helps schools improve operational efficiency, reduce administrative overhead, and make faster, data-driven decisions to support better academic management. 📊🎓",
    image: "/projects-4.webp",
    category: "Full Stack Developer",
    technologies: [
      "Next.js",
      "React",
      "TypeScript",
      "Prisma ORM",
      "MySQL",
      "Clerk Authentication",
      "Tailwind CSS",
      "React Hook Form",
      "Zod Validation",
      "Recharts",
      "React Big Calendar",
      "Moment.js",
      "RESTful API",
      "Laravel",
      "Vite",
      "Axios",
      "JWT / Session-based Auth",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "Authentication & role-based access control (Admin, Teacher, Staff)",
      "Student management (add, update, delete, class assignment)",
      "Teacher & staff management",
      "Class & academic structure management",
      "Real-time student count per class",
      "Weekly attendance tracking & recap",
      "Monthly payment statistics & reports",
      "Interactive academic calendar",
      "Dashboard with charts & analytics",
      "Form validation with real-time feedback",
      "Centralized academic & administrative reports",
      "Responsive dashboard layout",
    ],
    role: "Full Stack Developer",
  },
  {
    id: 5,
    title: "Inventory Management System 📦",
    description:
      "A web-based inventory management system that helps businesses manage stock, monitor orders, track user activities, and analyze revenue through a centralized real-time dashboard.",
    longDescription:
      "Inventory Management System is a web-based platform designed to streamline and centralize inventory and business operations in real time. The system enables efficient item and stock management, real-time order tracking, user activity monitoring, and revenue analysis through an intuitive dashboard. With quick access to reports, system settings, and performance insights, businesses can maintain accurate data, optimize workflows, and make faster, data-driven decisions to improve overall operational efficiency. 🚀",
    image: "/projects-5.webp",
    category: "Full Stack Developer",
    technologies: [
      "Laravel",
      "Inertia.js",
      "Vue.js 3",
      "Vite",
      "Tailwind CSS",
      "Axios",
      "Day.js",
      "RESTful API",
      "MVC Architecture",
      "Role-Based Access Control",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "User authentication & role-based access control",
      "Inventory & stock management",
      "Real-time stock level monitoring",
      "Item & category management",
      "Order and transaction tracking",
      "Revenue and sales analytics dashboard",
      "User activity and system log monitoring",
      "Centralized dashboard with key business metrics",
      "Report generation and data export",
      "Responsive UI for desktop and mobile devices",
    ],
    role: "Full Stack Developer",
  },
  {
    id: 6,
    title: "DESAKU – Village Information System 📊",
    description:
      "DESAKU is a web-based village information system that centralizes administrative services, citizen data, and reporting to improve transparency and public service efficiency.",
    longDescription:
      "DESAKU is a web-based Village Information System built to streamline and digitalize village administrative processes. The platform centralizes population data, public services, and reporting into a single integrated system, enabling village officials to manage data efficiently and transparently. With features such as a real-time dashboard, integrated citizen records, online complaint submissions, digital account requests, and automated reporting, DESAKU supports village digital transformation and improves the quality and efficiency of public services. 📊",
    image: "/projects-6.webp",
    category: "Full Stack Developer",
    technologies: [
      "Laravel",
      "Vite",
      "JavaScript",
      "Blade Templating",
      "Tailwind CSS",
      "Axios",
      "RESTful API",
      "MySQL",
      "MVC Architecture",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "User authentication & role-based access control",
      "Centralized citizen data management",
      "Village administrative service management",
      "Online public complaint submission",
      "Digital account and document request system",
      "Real-time dashboard for village statistics",
      "Automated reports and data recaps",
      "Responsive interface for desktop and mobile",
      "Secure data handling and validation",
    ],
    role: "Full Stack Developer",
  },
  {
    id: 7,
    title: "Bookshelf REST API 📚",
    description:
      "A RESTful Bookshelf API with comprehensive Postman test collections to validate endpoints, response structure, and business logic for book management features.",
    longDescription:
      "This project is a RESTful Bookshelf API designed to manage book data efficiently, including creating, retrieving, updating, and deleting books with reading and finished status filters. The API is thoroughly tested using Postman, featuring structured request collections and automated test scripts to validate HTTP status codes, response headers, data types, and business rules. Each endpoint is verified to ensure consistent JSON responses, correct properties, and accurate filtering logic. This project demonstrates strong backend fundamentals, API design best practices, and quality assurance through systematic API testing. 📚",
    image: "/projects-7.webp",
    category: "Backend Developer",
    technologies: [
      "Laravel",
      "Vite",
      "JavaScript",
      "Blade Templating",
      "Tailwind CSS",
      "Axios",
      "RESTful API",
      "MySQL",
      "MVC Architecture",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "User authentication & role-based access control",
      "Centralized citizen data management",
      "Village administrative service management",
      "Online public complaint submission",
      "Digital account and document request system",
      "Real-time dashboard for village statistics",
      "Automated reports and data recaps",
      "Responsive interface for desktop and mobile",
      "Secure data handling and validation",
    ],
    role: "Backend Developer",
  },
  {
    id: 8,
    title: "Portofolio V2 🔥",
    description:
      "A modern and responsive personal portfolio showcasing my journey as a Front End Developer, featuring selected projects, skills, and experience with a clean and elegant UI.",
    longDescription:
      "Portofolio V2 is the second iteration of my personal portfolio website, built to showcase my growth as a Front End Developer through a clean, modern, and responsive interface. This version focuses on delivering a smooth user experience, well-structured layouts, and visually consistent design while highlighting selected projects, technical skills, and professional background. Developed using modern frontend technologies, this portfolio reflects my commitment to writing clean, maintainable code and continuously improving my skills as I explore new tools, patterns, and best practices in frontend development. 🔥",
    image: "/projects-8.webp",
    category: "Frontend Developer",
    technologies: [
      "React",
      "Vite",
      "JavaScript",
      "Tailwind CSS",
      "Framer Motion",
      "GSAP",
      "AOS Animation",
      "React Router",
      "Responsive Web Design",
      "Git & GitHub",
    ],
    status: "Completed",
    year: "2025",
    features: [
      "Modern and responsive personal portfolio design",
      "Smooth animations using Framer Motion, GSAP, and AOS",
      "Interactive UI components with React",
      "Project showcase section with detailed descriptions",
      "Skills, experience, and education sections",
      "GitHub contribution calendar integration",
      "SEO optimization with React Helmet Async",
      "Fast development and build process using Vite",
      "Fully responsive layout for desktop and mobile devices",
    ],
    role: "Frontend Developer",
  },
  {
    id: 9,
    title: "Gym SaaS – Multi-Tenant Gym Management System 🏋️‍♂️",
    description:
      "A production-ready multi-tenant Gym Management SaaS platform built to manage members, attendance, classes, staff, and revenue with secure role-based access and scalable architecture.",
    longDescription:
      "Gym SaaS is a full-stack multi-tenant Gym Management System designed to help gym businesses efficiently manage daily operations in a single, scalable platform. The system includes member management with QR-based check-in, attendance tracking, membership plans, class scheduling, staff management, and real-time revenue analytics. Built using modern backend and frontend technologies, this project emphasizes clean architecture, service-layer pattern, secure authentication, and optimized database performance. It simulates a real-world SaaS product with production-grade security, role-based access control, and complete tenant data isolation.",
    image: "/projects-9.webp",
    category: "Full-Stack Developer",
    technologies: [
      "Laravel",
      "PHP",
      "PostgreSQL",
      "Laravel Sanctum",
      "Next.js",
      "React",
      "TypeScript",
      "Tailwind CSS",
      "TanStack Query",
      "Zustand",
      "RESTful API",
      "Multi-Tenant Architecture",
    ],
    status: "Completed",
    year: "2026",
    features: [
      "Multi-tenant architecture with full data isolation per gym",
      "Role-based access control (Owner, Staff, Trainer)",
      "Member management with auto-generated ID & QR code check-in",
      "Attendance tracking with real-time stats and duration calculation",
      "Flexible membership plans with billing periods and class credits",
      "Class scheduling with trainer assignment and capacity control",
      "Revenue dashboard analytics (daily, monthly, yearly)",
      "50+ RESTful API endpoints with service-layer architecture",
      "Secure authentication, validation, and optimized database queries",
    ],
    role: "Full-Stack Developer",
  },
];
